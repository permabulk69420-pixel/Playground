  ...
  const mesh=new THREE.Mesh(geo,new THREE.ShaderMaterial({uniforms,
    ...
    fragmentShader:`
      ...
      void main(){
        ...
        vec3 color=mix(vec3(1.6,.22,.01),vec3(2.8,1.4,.3),core);
        float alpha=(core*.75+halo*.25)*(0.35+flow*.65)*fade*(.45+motion*.4);
...
export function createBlastAtmosphere(scene) {
  ...
  const streams=makePalmStreams(scene);
  ...
  function reset(){fire.reset();smoke.reset();streams.reset();emission=0;for(const f of lights){f.life=0;f.light.intensity=0;}}
